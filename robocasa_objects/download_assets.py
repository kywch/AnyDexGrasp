# Original script: https://github.com/robocasa/robocasa/blob/main/robocasa/scripts/download_kitchen_assets.py
...
DOWNLOAD_ASSET_REGISTRY = dict(
    objaverse=dict(
        ...
        folder="assets/objects/objaverse",
    ...
    aigen_objs=dict(
        ...
        folder="assets/objects/aigen_objs",
        check_folder_exists=False,
    ),
)
...
def download_and_extract_zip(
    ...
    download_path = os.path.join(download_dir, "{}.zip".format(os.path.basename(folder)))
...
def download_kitchen_assets():
    ans = input("The script will download ~2 Gb of data. Proceed? (y/n) ")
    if ans == "y":
        print("Proceeding...")
    else:
        print("Aborting.")
