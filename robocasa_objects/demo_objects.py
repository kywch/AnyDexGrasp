# Original script: https://github.com/robocasa/robocasa/blob/main/robocasa/demos/demo_objects.py
...
def read_model(
    ...
    model_dir = os.path.dirname(abs_filepath)

    print(f"[read_model] Initial filepath: {filepath}")
    print(f"[read_model] Absolute filepath: {abs_filepath}")
    print(f"[read_model] Original CWD: {original_cwd}")
    print(f"[read_model] Target model CWD: {model_dir}")

    # NOTE: xml assets are defined in the relative manner, so chdir needed.
    os.chdir(model_dir)
    print(f"[read_model] CWD after chdir: {os.getcwd()}")

    with open(abs_filepath, "r") as file:  # Use absolute path for reading
        xml = file.read()
    ...
    site_list = find_elements(root, tags="site", return_first=False) or []
    if show_bbox:
        sites = {}
    ...
    # Restore original CWD, though it might not be strictly necessary for this script's main loop
    # as the next call to read_model will set it again.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mjcf",
        # default=None,
        default="/workspace/realdex/AnyDexGrasp/assets/objects/AGOD/97792/model.xml",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="agod",
        choices=["agod", "objaverse"],
        help="(optional) object types. choose among [objaverse, agod]",
    )
    ...
    obj_registry = args.obj_types

    agod_dir = os.path.join(BASE_ASSET_ZOO_PATH, "AGOD")
    agod_list = os.listdir(agod_dir)
    agod_list.sort()
    agod_idx = 0
    ...
    while True:
        if args.mjcf is not None:
            ...
            # Reload the viewer with the same filepath
            print(f"Refresh: Showing {filepath}")

        elif args.obj_types == "agod":
            if agod_idx >= len(agod_list):
                agod_idx = 0

            filepath = os.path.join(agod_dir, agod_list[agod_idx])
            filepath = os.path.join(filepath, "model.xml")
        ...
        if args.mjcf is None:
            print()
            print(colored("(Press N to view another object, R to refresh, Q to end demo)", "yellow"))
        else:
